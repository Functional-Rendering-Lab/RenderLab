...
{
    private static void LeftLabel(string label)
    {
        ImGui.Text(label);
        ImGui.SameLine();
    }
    ...
    public static int ComboEdit(string label, int index, string[] items)
    {
        LeftLabel(label);
        ImGui.Combo($"##{label}", ref index, items, items.Length);
        return index;
    }
}
