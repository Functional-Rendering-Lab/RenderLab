...
using var window = DesktopWindow.Create("RenderLab — M3 Deferred", WindowWidth, WindowHeight);
var vk = Vk.GetApi();
using var gpu = VulkanDevice.Create(vk, window.GetRequiredVulkanExtensions(),
    instance => window.CreateVulkanSurface(instance));

// ─── Upload mesh to GPU ──────────────────────────────────────────────

var (vertexBuffer, vertexMemory) = VulkanBuffer.Create<Vertex3D>(gpu, BufferUsageFlags.VertexBufferBit,
    mesh.Vertices);
var (indexBuffer, indexMemory) = VulkanBuffer.Create<uint>(gpu, BufferUsageFlags.IndexBufferBit,
    mesh.Indices);
...
var gbufferRenderPass = VulkanPipeline.CreateGBufferRenderPass(gpu);
var lightingRenderPass = VulkanPipeline.CreateOffscreenRenderPass(gpu, VulkanPipeline.HdrFormat);
var tonemapRenderPass = VulkanPipeline.CreateRenderPass(gpu);  // writes to swapchain (PresentSrc)
var overlayRenderPass = VulkanPipeline.CreateOverlayRenderPass(gpu);

// ─── Descriptor set layouts ──────────────────────────────────────────

var gbufferDsLayout = VulkanDescriptors.CreateGBufferSamplerLayout(gpu); // 3 samplers for lighting
var singleDsLayout = VulkanDescriptors.CreateSamplerLayout(gpu);         // 1 sampler for tonemap
...
// Debug visualization pipeline (same layout as tonemap but with push constants)
var debugVizFragModule = VulkanPipeline.CreateShaderModule(gpu, LoadSpv("debugviz.frag.spv"));
var debugVizPipeline = VulkanPipeline.CreateFullscreenPipeline(
    ...
    out var debugVizPipelineLayout);
...
var vizMode = VisualizationMode.Final;
...
var overlayFramebuffers = Array.Empty<Framebuffer>();
...
// Debug viz: one descriptor set per visualizable buffer (position, normal, albedo, depth, hdr)
var debugVizDescPool = default(DescriptorPool);
var debugVizPositionSets = Array.Empty<DescriptorSet>();
var debugVizNormalSets = Array.Empty<DescriptorSet>();
var debugVizAlbedoSets = Array.Empty<DescriptorSet>();
var debugVizDepthSets = Array.Empty<DescriptorSet>();
var debugVizHdrSets = Array.Empty<DescriptorSet>();

CreateTransientResources();

// ─── ImGui + GPU timestamps ──────────────────────────────────────────

var imgui = VulkanImGui.Create(gpu, overlayRenderPass);
var timestamps = GpuTimestamps.Create(gpu, 4); // GBuffer, Lighting, Tonemap, ImGui
