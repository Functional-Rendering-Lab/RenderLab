...
public static class ProjectIO
{
        ...
        return resolved.Match(
            ok: absolute =>
            {
                ...
                try
                {
                    ...
                    var doc = JsonSerializer.Deserialize<SceneDocument>(bytes, JsonOptions)
                        ?? throw new InvalidDataException("scene deserialised to null");
                    return ValidateSceneIndices(doc, absolute);
                }
                catch (JsonException ex)
                {
    ...
    private static Result<SceneDocument, ProjectError> ValidateSceneIndices(SceneDocument doc, string path)
    {
        ...
        {
            if (doc.Assets.Materials[i] is BlinnPhongMaterialDoc bp && bp.AlbedoMap is int amIdx)
            {
