...
How and where we document RenderLab.
Two channels: **markdown files** for decisions, **XML comments** for API.
...
| `PROJECT-MODEL.md` | Why and how `code/projects/` + `IPipeline` + `Application` replaced the per-demo composition roots |
| `ADDING-A-PAPER.md` | How to add a new paper implementation |
| `HOT-SHADER-RELOAD.md` | How shader hot reload works, what it covers, what it does not |
| `QOL-STRATEGY.md` | Tooling, developer experience, debug workflows |
...
- Lead with current state, not history.
  A reader should get the answer in the first paragraph.
- Use tables for comparisons (option A vs option B).
- Code blocks for architecture diagrams (ASCII art, not images).
- Keep documents updated when the state they describe changes.
...
Follow the **80/20 rule**: document the 20% of public API surface that carries 80% of the understanding.
Not every public member needs a doc comment.
...
- `<summary>` is one sentence, rarely two.
  Start with a verb or noun, not "This method..." or "Gets or sets...".
...
- No `<remarks>` essays.
  If it takes a paragraph to explain, it belongs in a `docs/*.md` file, not inline.
...
- When you complete a migration phase, update the document that describes it.
- When you add a new public type to `RenderLab.Gpu` or `RenderLab.Graph`, add the `<summary>`.
- When a doc comment becomes wrong, fix or delete it.
